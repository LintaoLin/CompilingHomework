...
public class DrawView extends View {
        ...
        if (viewParamsArrayList != null && viewParamsArrayList.size() > 0) {
            ...
            for (int i = 0; i < viewParamsArrayList.size(); i++) {
                ...
                for (int j = 0; j < (points[i].length / 2); j++) {
                    try {
                        points[i][j * 2] =
                            (float) (origin.getX() + scale.getScaleX() * countExprValue(
                                params.getxExprTree(), paramT.getStart() + params.getStepValue() * j));
                        points[i][j * 2 + 1] =
                            (float) (origin.getY() + scale.getScaleY() * countExprValue(
                                params.getyExprTree(), paramT.getStart() + params.getStepValue() * j));
            ...
            if (WordScanner.operationArray.contains(exprTree.getTokenType())) {
                ...
                if (operationNode != null && operationNode.isValiable()) {
                    result += countExprValue(exprTree.getTokenType(), operationNode.getLeftParam(),
                        operationNode.getRightParam(), value);
                } else {
                ...
                result +=
                    (Double) funcNode.getFunction().apply(countExprValue(funcNode.getChild(), value));
    ...
    double countExprValue(WordScanner.TokenType tokenType, ExprTree left, ExprTree right, double value)
